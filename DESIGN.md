...
> 四文档分工：**DESIGN.md=设计依据**（稳定，低频改）；**README.md=路线图与进度**（唯一进度源）；**AGENTS.md=agent 行为规则**（工具链命令与 lore 协议）；**TUTOR.md=教研员角色**（agent 陪跑学习的教学契约，由 AGENTS.md 路由触发）。
...
| 新增教研员陪跑（TUTOR.md + AGENTS.md 路由触发） | 学习者拉取仓库即得 agent 陪跑，零额外配置；教学/维护双模式显式分离防人设互窜；教案=各章 README/notes 单一数据源，不在 TUTOR.md 重复章内容 |
| 放宽教研员边界：学习者笔记维护与小脚本执行留在教研员模式 | 概念学习常伴随笔记整理（mark.md）与演示脚本，「碰仓库即退出」摩擦大；不泄答案、不动教材/参考实现、零 git 写操作三条保持不变 |
...
- **agent 陪跑（教研员）**：在本仓库打开任意兼容 AGENTS.md 的 agent，说明学习者身份（如「我是学习者，想学 t2」），agent 按 TUTOR.md 扮演教研员：预习引导 → 陪自写 → 跑章内测试验收 → 通过后对照复盘，不动教材与参考实现、零 git 写操作。
- 个人笔记默认在学习者自己的外部项目维护；也可让教研员直接维护章节目录下的笔记文档（如 `mark.md`）并跑演示小脚本。
